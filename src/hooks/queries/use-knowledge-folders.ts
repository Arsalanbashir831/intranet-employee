...
function flattenAndFilterTree(
	...
	const filteredItems: FolderTreeItem[] = [];
...
export function useKnowledgeFolders(page: number = 1, pageSize: number = 10, searchTerm: string = "") {
	const { user } = useAuth();
	
	return useQuery({
		queryKey: knowledgeFoldersKeys.list({ page, pageSize, employeeId: user?.employeeId, searchTerm }),
		queryFn: async () => {
			try {
				...
				const allFolders = flattenAndFilterTree(treeData?.folders || [], searchTerm);
