	...
	const columns: ColumnDef<ExecutiveTask>[] = [
		{
			...
			cell: ({ row }) => (
				<span className="text-sm font-medium text-gray-900">{row.original.title}</span>
			),
		},
		{
			accessorKey: "description",
			header: ({ column }) => <CardTableColumnHeader column={column} title="Description" />,
			cell: ({ row }) => (
				<span className="text-sm text-gray-700 line-clamp-2">{row.original.description}</span>
			),
		},
		{
			accessorKey: "assignTo",
			header: ({ column }) => <CardTableColumnHeader column={column} title="Assign To" />,
			cell: ({ row }) => <AssignToAvatars employees={row.original.assignTo} />,
		},
		{
			accessorKey: "assignBy",
			header: ({ column }) => <CardTableColumnHeader column={column} title="Assign By" />,
			cell: ({ row }) => (
		...
		{
			id: "action",
			header: () => <span className="text-sm font-medium text-gray-700">Action</span>,
			cell: ({ row }) => (
				<Button
					variant="ghost"
					size="sm"
					...
					{paginatedTasks.length === 0 ? (
						...
						<div className="w-max min-w-full">
							<CardTable
								...
								onRowClick={(row) => handleRowClick(row.original as ExecutiveTask)}
								noResultsContent={
									<div className="text-center py-8 text-gray-500">
										{searchQuery
