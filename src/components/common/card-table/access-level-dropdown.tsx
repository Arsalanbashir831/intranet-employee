...
export function AccessLevelDropdown({
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger id="dropdown-trigger-access-level" asChild>
				<Button
					variant="outline"
					className={
						"gap-1 min-w-[160px] justify-between " + (className ?? "")
					}>
					Access Level <ChevronDown className="size-4" />
				</Button>
			</DropdownMenuTrigger>
			<DropdownMenuContent
				align="end"
				className="w-60 p-3 border-[#E2E4E9]"
				...
				<div className="space-y-2">
					<div className="flex items-center gap-2 py-1.5">
						<RadioGroup
							value={allSelected ? "all" : "custom"}
							onValueChange={(v) => (v === "all" ? onChange([]) : null)}
